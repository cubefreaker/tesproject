...
<section>
    ...
    <div class="contentpanel cs_df">   
        ...
        <div class="panel-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><strong> 
                                ...
                                Email : {{ List[0].LfrRequest.Contact.Email }} <br/> 
                                Phone:  {{ List[0].LfrRequest.Contact.MobilePhone }}
                            </strong></td>
                        </tr>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr ng-repeat="FlightDetail in List[0].FlightDetails">
                            ...
                            <td> {{ FlightDetail.Category ? FlightDetail.Category : 'Economy' }} </td>
                        </tr>
            ...
            <!-- Discount Detail -->
            <div class="bg-primary" ng-if="List[0].UseDiscount">
                <strong>Discount Detail</strong>
            </div>
            <div class="table-responsive" ng-if="List[0].UseDiscount">
                <table class="table">
                    <tbody>
                        <tr ng-if="List[0].ReffId">
                            <td style="width: 10%"><strong> Discount Code </strong></td>
                            <td style="width: 20%"><strong> {{ List[0].DiscountDetail.DiscountCode }} </strong></td>
                        </tr>
                        <tr>
                            <td><strong> Discount Type </strong></td>
                            <td><strong> {{ List[0].DiscountDetail.DiscountType }} </strong></td>
                        </tr>
                        <tr>
                            <td><strong> Discount Nominal </strong></td>
                            <td><strong> {{ List[0].DiscountDetail.DiscountNominal }} </strong></td>
                        </tr>
                    </tbody>
                </table>
            </div>
  ...
  app.controller('TransactionController', function (AngularService, $scope, $filter, $window, $http, $timeout) {
    ...
    $scope.ShowIssuedButton = function()
    {
        if ($scope.List.length > 1) {
            if(
            $scope.List[0].PaymentStatusId == 3  && 
            $scope.List[0].IsTicketExpired == false &&
            $scope.List[1].PaymentStatusId == 3  && 
            $scope.List[1].IsTicketExpired == false &&  ($scope.List[0].RsvResponse.Status != 'Ticketed' ||  $scope.List[1].RsvResponse.Status != 'Ticketed')
            ) {
                return true;
            }
        }
            ...
            if(
                $scope.List[0].PaymentStatusId == 3 &&  
                $scope.List[0].RsvResponse.Status != 'Ticketed' && 
                $scope.List[0].IsTicketExpired == false){
                return true;
            }
        }
